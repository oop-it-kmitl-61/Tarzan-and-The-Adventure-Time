...
public class Player extends MapObject {
	...
	// animations all Length frame ขนาดกว้าง-ยาว **แต่ละ** เฟรม
	...
	public void update() {
		...
		if(currentAction == CHARGING) {
			...
			cr.y = (int)y - 20; //20
			if(facingRight) cr.x = (int)x - 15; //15
			else cr.x = (int)x - 35;//35
		}
...
//					}
			...
			if(currentAction != ATTACKING) {
				...
				ar.y = (int)y - 6; //6
				if(facingRight) ar.x = (int)x + 50;//10
				else ar.x = (int)x - 40; //40
			}
		}
		else if(charging) {
			if(currentAction != CHARGING) {
